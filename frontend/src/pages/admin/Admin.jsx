        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
          <div>
            <div className="flex items-center gap-2 mb-4">
              <ShieldCheck size={22} />
              <span className="uppercase tracking-widest text-sm opacity-80">ADMIN CONTROL CENTER</span>
            </div>
            <h1 className="text-3xl md:text-5xl font-bold mb-4 leading-tight">Welcome Back, Admin</h1>
            <p className="text-blue-100 max-w-2xl">
              Manage users, monitor payments, control pricing, and oversee all platform operations from one place.
            </p>
          </div>
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 min-w-[260px] border border-white/10">
            <p className="text-sm text-blue-100 mb-2">Pending Approvals</p>
            <h2 className="text-5xl font-bold mb-3">{stats.pendingPayments}</h2>
            <button
      ...
      {!loading && (
        <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
          {cards.map((card, index) => (
            <div key={index} className={`bg-gradient-to-br ${card.color} rounded-3xl p-6 text-white shadow-xl`}>
              <div className="flex items-center justify-between mb-6">
                <div className="bg-white/15 p-3 rounded-2xl">{card.icon}</div>
              </div>
      ...
      <div className="mb-10">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">Fast access to admin operations</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-red-600 to-orange-500 text-white p-6 rounded-3xl shadow-xl">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-bold text-xl mb-1">Pending Payment Requests</h3>
                <p className="text-red-100">
                  You currently have {stats.pendingPayments} payment request(s) waiting for approval.
                </p>
              </div>
            </div>
            <button
...
function ActionCard({ title, desc, icon, color, onClick, badge }) {
  return (
    <div
      onClick={onClick}
